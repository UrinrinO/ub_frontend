...
/* ─── HTML processor
   Converts <img class="blog-diagram" data-caption="..."> to <figure> elements
   and ensures headings have IDs for anchor navigation.
──────────────────────────────────────────────────────────────────────────── */
...
export default function BlogPostPage() {
  ...
  useEffect(() => {
    ...
    blogApi
      .list(true, slug)
      .then((posts) => {
        if (posts.length === 0) setNotFound(true);
        else setPost(posts[0]);
      })
  ...
  useEffect(() => {
    ...
    setProcessedContent(processHTML(post.content));
  }, [post]);
  ...
  if (notFound) {
    return (
      <div className="pt-32 pb-20 text-center">
        <p className="text-foreground/40 font-mono">Post not found.</p>
        <Link to="/blog" className="mt-4 inline-block text-sm text-foreground/60 underline">
          Back to Blog
        </Link>
      </div>
    );
